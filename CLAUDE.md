...
./doc-gaps.sh --focus server      # Focus on server components
./doc-gaps.sh -v                  # Verbose — stream output live
```
...
| `doc-gaps` | `/doc-gaps [focus]` | Documentation audit — find missing component docs, update stale ones, ensure devguide/docs/ is complete |

## Debugging Rules

- **Listen to the user.** When the user suggests where a bug might be, investigate that direction FIRST before going down your own path. Don't dismiss user intuition.
- **Server-first debugging.** This is a server-authoritative game. When something looks wrong on screen, verify the server is sending correct data before touching client code. Write a test against the actual broadcast payload (`getPlayerStateJSON`, not `getPlayerState`) to confirm.
- **Watch for stale caches.** `game-engine.js` caches serialized state (`_cachedPlayerJSON`, `_cachedStateJSON`). Any function that mutates state referenced by these caches MUST call `_invalidateStateCache()`. If you add or modify tick-processing functions that change game objects, invalidate the cache.
- **Write a failing test first.** Don't iterate on fixes without a test that reproduces the bug. If your test passes but the bug is still visible, your test doesn't cover the actual problem.
...
ColonyGame/
  ...
  devguide/
    ...
    docs/                  # Per-component documentation (generated by /doc-gaps)
  .claude/skills/          # Automation skills
  autopilot.sh             # Shell automation script
  autopilot.ps1            # PowerShell automation script
  doc-gaps.sh              # Shell documentation audit script
  doc-gaps.ps1             # PowerShell documentation audit script
```
